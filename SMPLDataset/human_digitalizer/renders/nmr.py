...
from SMPLDataset.human_pose3d_estimators.geometry import mesh
...
class BaseSMPLRenderer(nn.Module):
    ...
    def cal_bc_transform(self, src_f2pts, dst_fims, dst_wims):
        ...
        occlusion_map = torch.zeros((bs, self.image_size * self.image_size), dtype=torch.float32, device=src_f2pts.device)
        # w, h = self.image_size, self.image_size
        # x = torch.arange(w).view(1, -1).expand(h, -1).float()
        # y = torch.arange(h).view(-1, 1).expand(-1, w).float()
        # x = 2 * x / (w - 1) - 1
        # y = 2 * y / (h - 1) - 1
        # grid = torch.stack([x, y], dim=0).float().to(src_f2pts.device)
        # grid = grid.unsqueeze(0).expand(bs, -1, -1, -1).permute(0, 2, 3, 1)
        ...
        for i in range(bs):
            ...
            occlusion_map[i, to_exist_mask] = 1.0

        T = T.view(bs, self.image_size, self.image_size, 2)
        occlusion_map = occlusion_map.view(bs, self.image_size, self.image_size)
        # mask = T == -2
        # newT = T.clone()
        # newT[mask] = grid[mask]
        # T = torch.clamp(-2, 2)

        return T, occlusion_map
...
class SMPLRenderer(BaseSMPLRenderer):
    def __init__(self, face_path="SMPLDataset/human_digitalizer/assets/smpl_faces.npy",
                 fim_enc_path="SMPLDataset/human_digitalizer/assets/mapper_fim_enc.txt",
                 uv_map_path="SMPLDataset/human_digitalizer/assets/mapper_uv.txt",
                 part_path="SMPLDataset/human_digitalizer/assets/smpl_part_info.json",
                 front_path="SMPLDataset/human_digitalizer/assets/front_body.json",
                 head_path="SMPLDataset/human_digitalizer/assets/head.json",
                 facial_path="SMPLDataset/human_digitalizer/assets/front_facial.json",
                 map_name="uv_seg", tex_size=3, image_size=256,
                 anti_aliasing=True, fill_back=False, background_color=(0, 0, 0),
                 viewing_angle=30, near=0.1, far=25.0,
                 has_front=False, top_k=5):
