...
class OpenPoseRunner(BasePoseEstimator):
    ...
    def run_over_paths(self, paths, height_size=368, use_fast=True, pose_smooth=True):
        """

        Run detector over all the images in one folder.

        Args:
            paths (str): the paths of images;
            height_size (int):
            use_fast (bool):
            pose_smooth (bool):
        ...
        dataloader = ImageFolderDataset(paths)
        all_img_names = dataloader.file_paths
