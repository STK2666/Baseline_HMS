...
from human_pose3d_estimators.geometry.keypoints import KEYPOINTS_FORMATER
from utils import load_toml_file

class SMPLifyRunner(BasePose3dRefiner):

    def __init__(self,
                 cfg_or_path, use_lbfgs=True,
    ...
    def __call__(self, keypoints, pred_camera, pred_betas, pred_pose, proc_kps=True, temporal=True):
        ...
        Args:
            ...
            pred_camera: torch.Tensor, (bs, 3), the camera output of the SMPLDataset or HMR;
            pred_betas: torch.Tensor, (bs, 10), the shape output of the SMPLDataset or HMR;
            pred_pose: torch.Tensor, (bs, 72), the pose output of the SMPLDataset or HMR;
