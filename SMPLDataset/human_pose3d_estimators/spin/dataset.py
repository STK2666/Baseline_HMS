...
def resize_img(img, scale_factor):
    new_size = (np.floor(np.array(img.shape[0:2]) * np.float32(scale_factor))).astype(int)
...
class InferenceDatasetWithKeypoints(Dataset):
    def __init__(self, images, boxes, keypoints, keypoint_formater, temporal=False, image_size=224):
