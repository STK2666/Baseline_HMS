...
class PixelwiseFlowPredictor(nn.Module):
    def __init__(self, block_expansion, num_blocks, max_features, num_regions, num_channels, scale_factor=1, region_var=0.01,
        ...
        hourglass_in_features = (num_regions + 1) * (num_channels * use_deformed_source + 1) + num_channels
        self.hourglass = Hourglass(block_expansion=block_expansion, in_features=hourglass_in_features, max_features=max_features, num_blocks=num_blocks)

        # Combination of unsupervised and geometry supervised flow
        total_hourglass_features = self.hourglass.out_filters
        num_flows = num_regions + 2
        self.mask = nn.Conv2d(total_hourglass_features, num_flows, kernel_size=(7, 7), padding=(3, 3))
    ...
    def forward(self, source_image, driving_region_params, source_region_params, driving_smpl, source_smpl, bg_params=None, source_smpl_rdr=None, driving_smpl_rdr=None, source_depth=None, driving_depth=None):
        ...
        sparse_motion = self.get_motion_params_flows(source_image, driving_region_params, source_region_params, bg_params=bg_params)
        uns_warped = self.get_deformed(source_image, sparse_motion)
        ...
        uns_input = torch.cat([uns_input, smpl_warped], dim=1)
        prediction = self.hourglass(uns_input)

        sparse_motion = F.interpolate(sparse_motion.reshape(B,2*(self.num_regions + 1),sparse_motion.shape[-2],sparse_motion.shape[-1]), size=(H,W), mode='bilinear', align_corners=False).reshape(B,-1,2,H,W)
        sparse_motion = torch.cat([sparse_motion, smpl_flow.unsqueeze(1).permute(0, 1, 4, 2, 3)], dim=1)
        ...
        # uns_flow_fine = F.interpolate(sparse_motion, size=(H,W), mode='bilinear')
        # prediction = F.interpolate(prediction, size=(H,W), mode='bilinear')
        # combined_inputs = torch.cat([prediction, gs_prediction], dim=1)
