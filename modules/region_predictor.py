...
def svd(covar, fast=False):
    if fast:
        ...
        if torch.isnan(covar).any() or torch.isinf(covar).any():
            covar = torch.nan_to_num(covar)
            print("================= Covar is NaN ===================")
...
class RegionPredictor(nn.Module):
    ...
    def __init__(self, block_expansion, num_regions, num_channels, max_features,
        ...
        self.regions = nn.Conv2d(in_channels=self.predictor.out_filters, out_channels=num_regions, kernel_size=(7, 7),
                                 padding=pad)
        # self.regions = StyledConv(in_channels=self.predictor.out_filters, out_channels=num_regions, kernel_size=7, style_dim=85, upsample=False, demodulate=False)
    ...
    def region2affine(self, region):
        ...
        mean = (region * grid).sum(dim=(2, 3)) # get x,y = E(X,Y)

        region_params = {'shift': mean}

        # PCA based on eigenvalue decomposition of covariance matrix
        if self.pca_based:
    ...
    def forward(self, x, smpl):
        ...
        # smpl = smpl.squeeze(-1)
        # prediction = self.regions(feature_map, smpl)

        prediction = self.regions(feature_map)

        # smpl nc1 c[cam;pose;shape]
        # if smpl != None:


        final_shape = prediction.shape # b, num_regions, h, w
        region = prediction.view(final_shape[0], final_shape[1], -1) # b, num_regions, h*w
        region = F.softmax(region / self.temperature, dim=2) # in h*w dimension
        region = region.view(*final_shape) # b, num_regions, h, w
