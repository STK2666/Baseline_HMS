...
from modules.new_conv import SMPLStyledConv2d, ModulatedConv2d, SMPLConv2d
...
def region2gaussian(center, covar, spatial_size):
    ...
    if type(covar) == float:
        ...
        try:
            ...
            # replaced = torch.inverse(replaced)
            covar_inverse = torch.linalg.pinv(covar).view(*shape)
...
class Decoder(nn.Module):
    ...
    def __init__(self, block_expansion, in_features, num_blocks=3, max_features=256, with_mid=False):
    ...
    def forward(self, x):
        ...
        for up_block in self.up_blocks:
            ...
            if self.with_mid:
                if up_block == self.up_blocks[-1]:
                    self.mid.append(out[:, :self.out_filters-self.in_filters])
                else:
                    self.mid.append(out)
...
class Hourglass(nn.Module):
    ...
    def __init__(self, block_expansion, in_features, num_blocks=3, max_features=256, with_mid=False):
        ...
        self.decoder = Decoder(block_expansion, in_features, num_blocks, max_features, with_mid=with_mid)
        self.out_filters = self.decoder.out_filters

    def forward(self, x):
        out = self.decoder(self.encoder(x))
        if self.with_mid:
            self.mid = self.decoder.mid
        return out


class Comb_DownBlock2d(nn.Module):
    def __init__(self, in_features, out_features, kernel_size=3):
        super(Comb_DownBlock2d, self).__init__()
        self.conv = SMPLConv2d(in_features, out_features, kernel_size=kernel_size)
        self.norm = BatchNorm2d(out_features, affine=True)
        self.pool = nn.AvgPool2d(kernel_size=(2, 2))

    def forward(self, x, smpl):
        out = self.conv(x, smpl)
        out = self.norm(out)
        out = F.relu(out)
        out = self.pool(out)
        return out
...
class Comb_Encoder(nn.Module):
    def __init__(self, block_expansion, in_features, num_blocks=3, max_features=256):
        ...
        down_blocks = []
        for i in range(num_blocks):
            down_blocks.append(Comb_DownBlock2d(in_features if i == 0 else min(max_features, block_expansion * (2 ** i)),
                                           min(max_features, block_expansion * (2 ** (i + 1))),
                                               kernel_size=3))
        self.down_blocks = nn.ModuleList(down_blocks)
    ...
    def forward(self, x, smpl):
        outs = [x]
        for down_block in self.down_blocks:
            outs.append(down_block(outs[-1], smpl))
        return outs


class Comb_UpBlock2d(nn.Module):
    def __init__(self, in_features, out_features, kernel_size=3):
...
class Comb_Decoder(nn.Module):
    def __init__(self, block_expansion, in_features, num_blocks=3, max_features=256):
        super(Comb_Decoder, self).__init__()
        up_blocks = []
        ...
        for i in range(num_blocks)[::-1]:
            ...
            out_filters = min(max_features, block_expansion * (2 ** i))
            # up_blocks.append(UpBlock2d(in_filters, out_filters, kernel_size=3, padding=1))
            up_blocks.append(Comb_UpBlock2d(in_filters, out_filters, kernel_size=3))

        self.up_blocks = nn.ModuleList(up_blocks)
        self.in_filters = in_filters
        self.out_filters = block_expansion + in_features

    def forward(self, x, uns, gs, smpl):
        uns = uns[::-1]
        gs = gs[::-1]
        out = x.pop()
        for up_block in self.up_blocks:
            out = out + uns.pop() + gs.pop()
            out = up_block(out, smpl)
            skip = x.pop()
            out = torch.cat([out, skip], dim=1)
...
class Comb_Hourglass(nn.Module):
    def __init__(self, block_expansion, in_features, num_blocks=3, max_features=256):
        super(Comb_Hourglass, self).__init__()
        self.encoder = Encoder(block_expansion, in_features, num_blocks, max_features)
        # self.encoder = Comb_Encoder(block_expansion, in_features, num_blocks, max_features)
        self.decoder = Comb_Decoder(block_expansion, in_features, num_blocks, max_features)
        self.out_filters = self.decoder.out_filters


    def forward(self, x, uns, gs, smpl):
        # out = self.encoder(x, smpl)
        out = self.encoder(x)
        out = self.decoder(out, uns, gs, smpl)
        return out
